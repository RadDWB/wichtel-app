...
export async function setParticipantSession(sessionId, sessionData, ttlSeconds = 60 * 60 * 24 * 7) {
  try {
    const client = await getRedisClient();
    await client.setEx(`session:participant:${sessionId}`, ttlSeconds, JSON.stringify(sessionData));
    return true;
  } catch (error) {
...
export async function getParticipantSession(sessionId) {
  try {
    const client = await getRedisClient();
    const data = await client.get(`session:participant:${sessionId}`);
    return data ? JSON.parse(data) : null;
  } catch (error) {
    console.error('Error getting participant session:', error);
    return null;
  }
}
