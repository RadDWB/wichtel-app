            ...
            <div className="flex gap-3 mb-6">
              <button
                onClick={() => {
                  if (orgParticipant) {
                    const isPublicPairings = group?.settings?.pairingVisibility === 'public';
                    const dashboardUrl = isPublicPairings ? `/${groupId}/pairings` : (organizerPin ? `/organizer/${groupId}?showPin=${organizerPin}` : `/organizer/${groupId}`);
      ...
      <div className="min-h-screen bg-gradient-to-br from-amber-50 via-white to-red-50">
        <div className="container mx-auto py-12 px-4 max-w-2xl">
          <div className="bg-white rounded-lg p-8 shadow-lg mb-6">
            ...
            <div className="flex gap-3">
              {orgParticipant && (
                <Link href={group?.settings?.pairingVisibility === 'public' ? `/${groupId}/pairings` : (organizerPin ? `/organizer/${groupId}?showPin=${organizerPin}` : `/organizer/${groupId}`)} className="flex-1 text-center block p-3 bg-green-500 hover:bg-green-600 text-white rounded-lg font-semibold transition">
                  📊 Zum Dashboard
                </Link>
              )}
        ...
        <div className="container mx-auto py-12 px-4">
          <div className="max-w-2xl mx-auto">
            ...
            <div className="mt-8 pt-6 border-t border-gray-200 space-y-3">
              {/* Link to public pairings page - available for all in public mode */}
              {group?.settings?.pairingVisibility === 'public' && (
                <Link href={`/${groupId}/pairings`} className="block w-full text-center p-3 bg-blue-500 hover:bg-blue-600 text-white rounded-lg font-semibold transition">
                  🎁 Alle Paarungen anschauen
                </Link>
              )}
              ...
              {orgParticipant && (
                <>
                  ...
                  <Link href={group?.settings?.pairingVisibility === 'public' ? `/${groupId}/pairings` : (organizerPin ? `/organizer/${groupId}?showPin=${organizerPin}` : `/organizer/${groupId}`)} className="block w-full text-center p-3 bg-gradient-to-r from-green-600 to-green-700 hover:from-green-700 hover:to-green-800 text-white rounded-lg font-semibold transition">
                    📊 Zum Organizer Dashboard
                  </Link>
                </>
              )}
            </div>
