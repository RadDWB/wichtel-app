      ...
      <div className="min-h-screen bg-gradient-to-br from-amber-50 via-white to-red-50">
        <div className="container mx-auto py-12 px-4 max-w-2xl">
          <div className="bg-white rounded-lg p-8 shadow-lg mb-6">
            ...
            <div className="flex gap-3">
              {orgParticipant && (
                <Link href={group?.settings?.surpriseMode === 'mutual' && group?.settings?.pairingVisibility === 'public' ? `/${groupId}/pairings` : (organizerPin ? `/organizer/${groupId}?showPin=${organizerPin}` : `/organizer/${groupId}`)} className="flex-1 text-center block p-3 bg-green-500 hover:bg-green-600 text-white rounded-lg font-semibold transition">
                  📊 Zum Dashboard
                </Link>
              )}
